...
export const usePushNotifications = (): UsePushNotificationsReturn => {
    ...
    const hydrate = async () => {
      ...
      if (isNative) {
        try {
          ...
          const mappedPermission: NotificationPermission | 'default' =
          ...
          setIsSubscribed(subscribed && mappedPermission === 'granted');
        } catch (e) {
          console.error('Failed to check native push permission:', e);
  ...
  const checkWeatherAlerts = useCallback((forecast: any[], currentWeather?: any): WeatherAlert[] => {
    ...
    // Current weather checks (immediate alerts)
    if (currentWeather) {
      if (currentWeather.rainfall >= THRESHOLDS.HEAVY_RAIN_MM) {
        alerts.push({
          type: 'heavy_rain',
          title: `🌧️ ${t('heavyRainAlert')}`,
          message: `${t('heavyRainfallWarning')} (${currentWeather.rainfall}mm now)`,
      ...
      if (currentWeather.temperature >= THRESHOLDS.HEAT_TEMP_C) {
        alerts.push({
          type: 'heat',
          title: `🔥 ${t('heatAlert')}`,
          message: `${t('heatAlert')} - ${currentWeather.temperature}°C`,
      ...
      if (currentWeather.humidity >= THRESHOLDS.HIGH_HUMIDITY_PERCENT) {
        alerts.push({
          type: 'disease_risk',
          title: `🦠 ${t('diseaseRiskAlert')}`,
          message: `${t('diseaseRiskAlert')} - ${currentWeather.humidity}% humidity`,
      ...
      const currentWindKmh = Number(currentWeather.windSpeed || 0);
      if (currentWindKmh >= THRESHOLDS.STRONG_WIND_KMH) {
        alerts.push({
          type: 'strong_wind',
          title: `💨 ${t('windAlert')}`,
          message: `${t('windAlert')} - ${Math.round(currentWindKmh)} km/h`,
  ...
  }, [isNative, permission, swRegistration, t]);
