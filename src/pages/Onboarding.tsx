...
import { User, MapPin, Calendar, Navigation } from "lucide-react";
...
const Onboarding = () => {
  ...
  const { t } = useLanguage();
  const { user } = useAuth();
  const { profile: existingProfile, loading: profileLoading, saveProfile } = useFarmerProfile();
  ...
  const cropLabels = Object.fromEntries(crops.map((c) => [c.value, c.label]));
  const sizeLabels = Object.fromEntries(farmSizes.map((s) => [s.value, s.label]));
  ...
  const isFormValid =
    profile.name.trim() && profile.location.trim() && profile.farms.length > 0 && profile.farmingSeason;
        ...
        <div className="mx-auto max-w-md space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setProfile({ ...profile, farmingSeason: value })
              }
            >
              <SelectTrigger>
                <SelectValue placeholder={t("farmingSeason")} />
              </SelectTrigger>
              <SelectContent>
          ...
          <AddFarmDialog onAdd={handleAddFarm} crops={crops} farmSizes={farmSizes} />
